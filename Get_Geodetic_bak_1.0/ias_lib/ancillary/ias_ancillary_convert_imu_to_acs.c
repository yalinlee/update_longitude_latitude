...
/* Number of SIRU synch errors allowed in an interval */
#define SYNCH_ERR_TOL 3  /*modified by guoshaofeng 20130903*/
...
)
{
    ...
    int max_consecutive_invalid_count; /* maximum consecutive invalid IMU point
                                          count */
    IAS_L0R_IMU *fixed_imu;          /* IMU data that has any anomalies fixed */
    ...
    fixed_imu = ias_ancillary_process_imu_times(source_l0r_imu,
	    ancillary_eng->gyro_conv_time, l0r_imu_count, imu_valid, imu_time_data);
    if (fixed_imu == NULL)
    {
	    IAS_LOG_ERROR("Calculating IMU times");
	    return ERROR;
    }/* modified by guoshaofeng 20130903*/
    ...
    if (*imu_valid)
    {
        ...
        last_angle = &(fixed_imu[0].gyro_samples[0]);
             ...
             index++)
        {
            ...
            for (sub_index = 0; sub_index < IAS_L0R_NUM_SAMPLES_PER_IMU_RECORD;
                 sub_index++, imu_curr++)
            {
                curr_angle = &(fixed_imu[index].gyro_samples[sub_index]);
                        ...
                        == SATURATION_AND_SCALING_VALID)
                {
                    ...
                    if (imu_last < 0)
                    {
                        ...
                        {
                        	last_angle =
                                &(fixed_imu[index].gyro_samples[sub_index + 1]);
                        }
                        else if (index < l0r_imu_count - 1)
                        {
                            last_angle =
                            	&(fixed_imu[index + 1].gyro_samples[0]);
                        }
                        else
                        {
...
Purpose: This routine checks for known anomalies in the ancillary data that can
         impact the SIRU data and works around the ones that it can.

Returns:
     SUCCESS:  Any anomalies found were able to be worked around
     ERROR:    An anomaly was found that could not be worked around.  The
               caller should consider all SIRU data invalid.

Notes:
    - There are three types of anomalies related to SIRU data that have been
      encountered.  The 684 bytes refers to the spacecraft data stream that is
      assembled into the mission data file.  The checks here account for where
      those bytes map to in our structures.
    - Type 1: An entire ancillary data frame was missed.  If this is found,
        there is no work around for it.  Consider all the SIRU data invalid
        when this routine returns ERROR.
    ...
    - Type 3: The third group of 684 bytes (bytes 1369-2052) are duplicated
        from previous frame.  The affected samples are flagged as invalid
        along with the first sample from the following record.

*****************************************************************************/
static int check_for_anomalies
(
    IAS_L0R_IMU *l0r_imu, /* I/O: Ancillary IMU records that can be updated if
                             anomalies are found */
    int imu_count,        /* I: Number of records present */
    int record_index,     /* I: Record index to check */
    int clocks_per_sample,/* I: SIRU clock counts in one sample */
    int clocks_per_record /* I: SIRU clock counts in one record */
)
{

    IAS_L0R_IMU_SAMPLE *curr_rec_samples;
    const IAS_L0R_IMU_SAMPLE *prev_rec_samples;
    int subfield = 0;
    int i;
    int predicted_clock;        /* Predicted SIRU clock at start of record */


    if (record_index <= 0)
    {
        /* Can't check the first record for anomalies */
        return SUCCESS;
    }

    /* Check for type 2 anomaly:  duplicate second block of 684 bytes from
       previous frame */
    curr_rec_samples = &(l0r_imu[record_index].gyro_samples[0]);
    prev_rec_samples = &(l0r_imu[record_index - 1].gyro_samples[0]);

    if (curr_rec_samples[0].time_tag == prev_rec_samples[0].time_tag
        && curr_rec_samples[1].time_tag == prev_rec_samples[1].time_tag)
    {
        IAS_LOG_WARNING("Duplicate times detected in SIRU data record index"
            " %d.  Deactivating first 7 SIRU samples.", record_index);
        for (i = 0; i < 6; i++)
        {
            predicted_clock = (prev_rec_samples[i].time_tag
                + clocks_per_record) % IMU_MAX;
            curr_rec_samples[i].time_tag = predicted_clock;
            curr_rec_samples[i].angular_rate_valid = 0x00;
        }
        /* Deactivate the next sample also */
        curr_rec_samples[6].angular_rate_valid = 0x00;
    }

    /* Check for type 3 anomaly:  duplicate third block of 684 bytes from
       previous frame */
    if (curr_rec_samples[IAS_L0R_NUM_SAMPLES_PER_IMU_RECORD-1].time_tag
            == prev_rec_samples[IAS_L0R_NUM_SAMPLES_PER_IMU_RECORD-1].time_tag
        && curr_rec_samples[IAS_L0R_NUM_SAMPLES_PER_IMU_RECORD-2].time_tag
            == prev_rec_samples[IAS_L0R_NUM_SAMPLES_PER_IMU_RECORD-2].time_tag)
    {
        IAS_LOG_WARNING("Duplicate times detected in SIRU data record index"
            " %d.  Deactivating last 45 SIRU samples.", record_index);

        /* Deactivate the previous sample also */
        curr_rec_samples[5].angular_rate_valid = 0x00;
        for (i = 6; i < IAS_L0R_NUM_SAMPLES_PER_IMU_RECORD; i++)
        {
            predicted_clock = (prev_rec_samples[i].time_tag
                + clocks_per_record) % IMU_MAX;
            curr_rec_samples[i].time_tag = predicted_clock;
        ...
        /* Flag off the first sample in the next record also since delta angles
           will not be correct */
        if ((record_index + 1) < imu_count)
            l0r_imu[record_index + 1].gyro_samples[0].angular_rate_valid = 0x00;
    }

    /* Check for type 1 anomaly:  missing ancillary data frame */
    predicted_clock = (prev_rec_samples[subfield].time_tag + clocks_per_record)
        % IMU_MAX;
    predicted_clock = abs(predicted_clock
        - curr_rec_samples[subfield].time_tag);

    if (predicted_clock > (IMU_MAX / 2))
        predicted_clock = abs(predicted_clock - IMU_MAX);

    if (predicted_clock > 2 * clocks_per_sample)
    {
        /* See if an entire record was skipped */
        predicted_clock = (prev_rec_samples[subfield].time_tag
            + 2 * clocks_per_record) % IMU_MAX;
        predicted_clock = abs(predicted_clock
            - curr_rec_samples[subfield].time_tag);
        ...
        if (predicted_clock < 2 * clocks_per_sample)
        {
            IAS_LOG_WARNING("Missing ancillary record detected in SIRU data at"
                " record index %d", record_index);
        }
        else
        {
            IAS_LOG_WARNING("Time discontinuity detected in SIRU data at"
                " record index %d", record_index);
        }

        /* Return an ERROR to indicate the anomaly found could not be worked
           around */
        return ERROR;
    }
...
Purpose: Process the IMU time data -- check the L0R IMU synchronization data, 
         correcting the data if a re-sync occurred.  A separate copy of the
         data with any corrections is returned.  The caller needs to free the
         returned pointer.

Returns:
         NULL:     Operation failed
         non-null pointer with corrections:  Successful completion
*****************************************************************************/
IAS_L0R_IMU *ias_ancillary_process_imu_times/* modified by guoshaofeng 20130903*/
(
...
)
{
    ...
    int clock_samp;         /* SIRU clock counts between samples */
    int clock_rec;          /* SIRU clock counts between ancillary records */

    int num_synch_err = 0;  /* Number of clock synch errors detected */
    int *imu_clock_samples; /* Temporary array to hold SIRU clock samples */
    IAS_L0R_IMU *fixed_imu; /* Copy of IMU data with corrections applied */
    const IAS_L0R_IMU_SAMPLE *curr_angle;

    /* Assume the data will be invalid */
    *imu_valid = 0;

    /* Allocate an array to hold the IMU data with any corrections applied */
    fixed_imu = malloc(sizeof(*fixed_imu) * imu_count);
    if (fixed_imu == NULL)
    {
        IAS_LOG_ERROR("Allocating fixed SIRU sample buffer");
        return NULL;
    }

    /* Copy the source IMU data to the new buffer where it might be getting
       modified */
    memcpy(fixed_imu, source_l0r_imu, sizeof(*fixed_imu) * imu_count);
    ...
    /* Calculate the number of SIRU clock counts in one sample */
    clock_samp = (int)floor(IAS_ANCILLARY_IMU_TIME / imu_offset_scale + 0.5);

    /* Calculate the number of SIRU clock counts in one record */
    clock_rec = clock_samp * IAS_L0R_NUM_SAMPLES_PER_IMU_RECORD;
    ...
    for (field = 0, imu_curr = 0; field < imu_count; field++)
    {
        ...
        /* Check for any anomalies that are known to occur in the data and
           work around them if possible */
        if (check_for_anomalies(fixed_imu, imu_count, field, clock_samp,
            clock_rec) != SUCCESS)
        {
            /* All the SIRU data is being inactivated by returning without
               setting the imu_valid flag */
            IAS_LOG_WARNING("Deactivating all SIRU data");
        ...
        curr_time = IAS_L0R_CONVERT_TIME_TO_SECONDS_SINCE_J2000(
            fixed_imu[field].l0r_time);
        for (subfield = 0; subfield < IAS_L0R_NUM_SAMPLES_PER_IMU_RECORD;
             subfield++, imu_curr++)
        {
            curr_angle = &(fixed_imu[field].gyro_samples[subfield]);
            ...
            if (curr_angle->sync_event_time_tag != IMU_FILL)
            {
                ...
                if (imu_curr > 0)
                {
                    imu_offset = (imu_clock_samples[imu_curr]
                        - imu_clock_samples[imu_curr - 1] + IMU_MAX) % IMU_MAX;


                    if (fabs(imu_time[imu_curr] - imu_time[imu_curr - 1]
                            - (imu_offset * imu_offset_scale))
            ...
            else /* sync time is fill */
            {
                ...
                {
                    /* Process the current SIRU clock sample */
                    imu_offset = (curr_angle->time_tag - base_sync + IMU_MAX)
                        % IMU_MAX;
                    ...
                    if (curr_time > (base_time + IAS_ANCILLARY_IMU_TIME)
                        && curr_time < imu_time[imu_curr])
                    {
                        sync_time = imu_time[imu_curr] - curr_time;
                        base_time = curr_time + sync_time;
    ...
    /* Make sure we did not exceed the allowable number of SIRU clock synch
       errors */
    if (num_synch_err > SYNCH_ERR_TOL)
    {
        /* All the SIRU data is being inactivated by returning without setting
           the imu_valid flag */
        IAS_LOG_WARNING("Number of SIRU clock synch errors found (%d) exceeds "
            "tolerance. SIRU data deactivated.", num_synch_err);
        free(imu_clock_samples);
        return fixed_imu;
    }
