...
NAME: ias_cpf_parse_tirs_detector_response

PURPOSE: Retrieve the tirs detector response data group from the odl tree and 
...
#define NUMBER_BAND_ATTRIBUTES 3
...
)
{
    ...
    char group_name[] = "TIRS_DETECTOR_RESPONSE"; 
    ...
    for (band_index = 0; band_index < nbands; band_index++)
    {
        ...
        if (ndet == ERROR)
        {
            IAS_LOG_ERROR("Getting detector count per sca group: %s", 
                            group_name);
            for (band_index = 0; band_index < nbands; band_index++)
            {
                for (sca_index = 0; sca_index < nscas; sca_index++)
                {
                    free(tirs_det_response
        ...
        for (sca_index = 0; sca_index < nscas; sca_index++)
        {
            ...
            tirs_det_response->baseline_dark_response[band_index][sca_index] 
                        = malloc(ndet * sizeof(double)); 
            if (tirs_det_response->baseline_dark_response[band_index][sca_index]
                    == NULL) 
            {
                IAS_LOG_ERROR("Allocating memory for tirs dark response "
                              "group: %s", group_name);
                for (band_index = 0; band_index < nbands; band_index++)
                {
                    for (sca_index = 0; sca_index < nscas; sca_index++)
            ...
            tirs_det_response->background_response[band_index][sca_index] 
                        = malloc(ndet * sizeof(double)); 
            if (tirs_det_response->background_response[band_index][sca_index]
                    == NULL) 
            {
            ...
            /* allocate space for Gain Function Offset parameters */
            tirs_det_response->gain_offsets[band_index][sca_index] 
                        = malloc(ndet * sizeof(double)); 
            if (tirs_det_response->gain_offsets[band_index][sca_index]
                    == NULL) 
            {
    ...
    /* Retrieve blackbody thermistor weights from odl tree */
    list[count].group_name = group_name;
    list[count].attribute = "Blackbody_Thermistor_Weights";
    list[count].parm_ptr = tirs_det_response->blackbody_thermistor_weights;
    list[count].parm_size = sizeof(double) * 4;
    list[count].parm_type = IAS_ODL_Double;
